...
//
// master:
// 	BenchmarkMessageWillBePosted/Basic-8         	   	18282	     63388 ns/op
// 	BenchmarkMessageWillBePosted/Long_from_github-8		  477	   2485982 ns/op
//
// This branch:
// 	BenchmarkMessageWillBePosted/Basic-8         	   	14000	     82091 ns/op
// 	BenchmarkMessageWillBePosted/Long_from_github-8       366	   3255855 ns/op

func BenchmarkMessageWillBePosted(b *testing.B) {
	links := []autolink.Autolink{
		{
			Pattern:  "(Mattermost1)",
			Template: "[Mattermost](https://mattermost.com)",
		}, {
			Pattern:  "(?P<key>Mattermost2)",
	...
	for _, tc := range []struct {
		...
		{
			"Basic",
			"Welcome to Mattermost1! MM-12345 should link to Jira!",
			"Welcome to [Mattermost](https://mattermost.com)! [MM-12345](https://mattermost.atlassian.net/browse/MM-12345) should link to Jira!",
		},
		{
			"Long from github",
			longTextFromGithub,
		...
		b.Run(tc.Name, func(b *testing.B) {
			...
			assert.Equal(b, tc.ExpectedMessage, post.Message)
		})
	}
}

var longTextFromGithub = strings.Join([]string{
	"# Contributing",
	"",
